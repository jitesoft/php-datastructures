...
trait MapMethodsTestTrait {
    ...
    public function testForEach() {
        ...
        $this->implementation->forEach(
            function($value, $key, $list) use(&$count) {
        ...
        $count = 0;
        Maps::forEach(
            $this->implementation, function($value, $key, $list) use(&$count) {
                $this->assertNotNull($value);
                $this->assertTrue(isset($list[$key]));
                $this->assertSame($value, $list[$key]);
                $count++;
    ...
    public function testForEachWithBreak() {
        ...
        $count = 0;
        Maps::forEach(
            $this->implementation, function($value) use(&$count) {
                if ($value === 'three') {
                    return false;
                }
                $count++;
    ...
    public function testMap() {
        ...
        $out   = $this->implementation->map(
            function($value, $key, $map) use(&$count) {
        ...
        $count = 0;
        $out2  = Maps::map(
            $this->implementation, function($value, $key, $map) use(&$count) {
                $this->assertEquals($value, $map[$key]);
                $this->assertNotNull($value);
    ...
    public function testMapArray() {
        ...
        $count = 0;
        $out   = Maps::map(
            $arr, function($value, $key, $map) use(&$count) {
                $this->assertEquals($value, $map[$key]);
                $this->assertNotNull($value);
